...
@section('content')
    <div class="breadcrumb mt-4 mb-4">
        ...
        <img class="h-24 me-3 ms-3" src="{{asset('web/image/icon-breadcrumb.png')}}" alt="no-icon">
        <span class="fs-14 fw-400 text-secondary">{{$alert->title??'غير محدد'}}</span>
    </div>
    <div class="card-border mt-16">
        <div class="d-flex justify-content-between">
            <h5 class="text-primary">{{$notification->title??'غير محدد'}}</h5>
        </div>
        <hr class="hr-card">
        <div class="card-details">
            ...
            <div class="row">
                <div class="col-lg-4 col-md-6 col-12 mb-3">
                    ...
                    <div class="d-flex">
                        <img class="image-table" src="{{getFile($notificationUser!==null?$notificationUser->image:'assets/uploads/avatar.png','assets/uploads/avatar.png')}}" alt="no-image">
                        <h6 class="name-table d-flex align-items-center">{{$notificationUser->full_name??'غير محدد'}}</h6>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 col-12 mb-3">
                    ...
                    <div class="table-info text-red">
                        ...
                        @elseif ($notification->priority == 'mid')
                ...
                <div class="col-lg-4 col-md-6 col-12 mb-3">
                    ...
                    <span class="status-new">
                                {{$notificationStatus??'غير محدد'}}
                            </span>
                </div>
                <div class="col-lg-4 col-md-6 col-12 mb-3">
                    <p class="text-secondary fs-14 fw-400 mb-2">تاريخ الارسال</p>
                    <p class="text-primary fs-14 fw-500">{{\Carbon\Carbon::parse($notification->created_at)->locale('ar')->translatedFormat('d F Y')??'غير محدد'}}</p>
                </div>
            </div>
        </div>
        <div class="row">
        ...
        <div class="card-details mt-16">
            ...
            <hr class="hr-card">
            <div class="row">
                @forelse($notification->media as $file)
                    <a href="{{getFile($file->file)}}" target="_blank" class="col-lg-6 col-12 mb-2">
                        <div class="d-flex bg-white rounded p-2">
                            <img class="h-40 ms-2" src="{{getFile($file->file)}}" alt="no-image">
                            <div>
                                <p class="text-primary fs-12 fw-400">{{ $file->file_name }}</p>
                                <p class="text-secondary fs-12 fw-400 mb-2">{{ getFileSizeInMB($file->file) }}</p>
                            </div>
                        </div>
                    </a>
                @empty
                    <div class="col-12">
                        <p class="text-secondary fs-14 fw-400 text-center">لا يوجد ملفات مرفقة</p>
                    </div>
                @endforelse
            </div>
        </div>
