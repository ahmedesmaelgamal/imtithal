...
@section('content')
    <div class="breadcrumb mt-4 mb-4">
        ...
        <img class="h-24 me-3 ms-3" src="{{asset('web/image/icon-breadcrumb.png')}}" alt="no-icon">
        <span class="fs-14 fw-400 text-secondary">تعديل الرحلة</span>
    </div>

    <div class="card-border mt-16">
        <div class="d-flex flex-column align-items-start mb-4">
            <h5 class="text-primary fs-18">تعديل رحلة</h5>
            <p class="text-secondary fs-14 fw-400">يرجى تعديل بيانات الرحلة</p>
        </div>
        <hr class="hr-card">
        <form id="editTripFormFull" class="row g-3" action="{{ route('trips.update', $trip->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">رقم الرحلة <span class="text-red">*</span></label>
                <input name="trip_number" type="text" class="form-control" value="{{$trip->trip_number}}" required>
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">الناقل الجوي <span class="text-red">*</span></label>
                <input name="air_carrier" type="text" class="form-control" value="{{$trip->air_carrier}}" required>
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">بلد المغادرة <span class="text-red">*</span></label>
                <input name="departure_country" type="text" class="form-control" value="{{$trip->departure_country}}" required>
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">رقم قائمة الجاهزية</label>
                <input name="readiness_list_number" type="text" class="form-control" value="{{$trip->readiness_list_number}}">
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">مقدم الخدمة</label>
                <input name="service_provider" type="text" class="form-control" value="{{$trip->service_provider}}">
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">عدد حجاج المجموعات</label>
                <input name="hajj_groups_count" type="number" class="form-control" value="{{$trip->hajj_groups_count}}">
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">إجمالي عدد الحجاج</label>
                <input name="hajjis_count" type="number" class="form-control" value="{{$trip->hajjis_count}}">
            </div>
            <div class="col-md-6 mb-3">
                ...
                <select name="area_id" class="form-select js-select2">
                    ...
                    @foreach($areas as $area)
                        <option value="{{ $area->id }}" {{ $trip->area_id == $area->id ? 'selected' : '' }}>{{ $area->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">مدينة السكن</label>
                <input name="residence_city" type="text" class="form-control" value="{{$trip->residence_city}}">
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">تاريخ الوصول</label>
                <input name="arrival_date" type="date" class="form-control" value="{{$trip->arrival_date}}">
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">وقت الوصول</label>
                <input name="arrival_time" type="time" class="form-control" value="{{$trip->arrival_time}}">
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">جهة التنفيذ</label>
                <input name="executor" type="text" class="form-control" value="{{$trip->executor}}">
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fs-14 fw-500 text-primary">رقم العقد</label>
                <input name="contract_number" type="text" class="form-control" value="{{$trip->contract_number}}">
            </div>
            </div>
            <div class="col-12 text-start mt-4">
                <button type="submit" class="main-button">حفظ التعديلات</button>
            </div>
        </form>
    </div>
@endsection
