...
@section('content')
    ...
    <div class="card-border mt-16">
        <div class="d-flex justify-content-between flex-wrap">
            <h5 class="text-primary">إدارة التقارير </h5>
            <div class="d-flex">
                ...
                <button type="button" class="main-button" data-bs-toggle="modal" data-bs-target="#axisReportModal">
                    طباعة التقارير
                </button>
            </div>
        </div>
        ...
        <div class="form-filter card-details mt-4 mb-4">
            <form class="row" action="{{ route('axesManagement') }}" method="get">
                ...
                <div class="col-md-6 col-12 mb-3">
                    <label for="validationDefault04" class="form-label fs-14 fw-500 text-primary">اسم السؤال
                        <span class="text-red">*</span>
                    </label>
                    <input type="text" style="background-color: white;"
                        class="form-control fs-12 fw-400 text-secondary bg-gray" name="num_of_questions"
                        value="{{ request('num_of_questions') }}" placeholder="أدخل اسم السؤال">
                </div>
                <div class="col-md-6 col-12 mb-3">
    ...
    <!-- Axis Report Modal -->
    <div class="modal fade" id="axisReportModal" tabindex="-1" aria-labelledby="axisReportModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="axisReportModalLabel">طباعة التقارير</h5>
                </div>
                <div class="modal-body">
                    <select class="form-select" name="axis_id" id="axisSelect" multiple>
                        @foreach ($axies as $axis)
...
@section('js')
    <script>
        $(document).on('click', '.axisReportPrint', function(e) {
            e.preventDefault();
            var form = $(this);
            var url = "{{ route('axisReportPrint') }}" + "?axis_id=" + $(this).data('id');
            $.ajax({
                url: url,
                type: 'GET',
    ...
    <script>
        $('#ImportReportsForm').on('submit', function (e) {
            ...
            let url = "{{ route('axesManagement.importReports') }}";
    ...
    <script>
        ...
        $(document).on('click', '#printSurveysReportBtn', function(e) {
            e.preventDefault();
            var $btn = $(this);
            var selectedAxes = $('#axisSelect').val();

            if (!selectedAxes || selectedAxes.length === 0) {
                toastr.warning('يرجى اختيار محور واحد على الأقل');
                return;
            }
            ...
            $.ajax({
                url: "{{ route('axisReportPrint') }}",
                type: 'GET',
                data: {
                    axis_id: selectedAxes,
                    order: $('#axisSelect').val() // إرسال الترتيب الحالي
                },
                success: function(response) {
                    if (response.status) {
                        var printWindow = window.open('', '_blank');
    ...
    <script>
        $(document).ready(function() {
            $('.dropify').dropify();
            // تهيئة Select2
            var $select = $("#axisSelect").select2({
                dropdownParent: $(".modal-content"),
                placeholder: ' اختر المحاور ',
                width: '100%',
                dir: 'rtl'
            });
